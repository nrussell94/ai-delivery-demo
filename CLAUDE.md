...
- **Lombok for `@Document` models.** Persistence classes under `model/` use `@Getter`, `@Setter`, and `@NoArgsConstructor` so field declarations stay the focus. Don't reach for `@Data` on entities — entities should be equal by `@Id`, not all-fields, and an autogenerated `@ToString` over a document leaks PII (e.g. `description`, `reportedBy`). Generated API DTOs in `com.example.demo.api.model.*` keep their generator-emitted getters/setters; don't lombokify them — they're overwritten on every `mvn generate-sources`.
