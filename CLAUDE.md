...
- **openapi-generator does not prune stale outputs** (retrospective, 2026-05-17). Removing a path or tag from `contracts/openapi.yaml` does NOT delete the corresponding `*Api.java` / model classes under `backend/target/generated-sources/openapi/`. They linger from the previous run, still compile, and pollute autocomplete. The generator only ever writes; it never deletes. If you have removed something from the contract, run `mvn -f backend/pom.xml clean generate-sources` rather than plain `generate-sources` to guarantee the generated tree matches the contract.
