...
- **Use `Instant`, not `OffsetDateTime`, for fields on `@Document` classes** (FNOL delivery, 2026-05-18). Spring Data MongoDB's `Jsr310Converters` register codecs for `Instant`, `LocalDateTime`, and `ZonedDateTime` but NOT for `OffsetDateTime`. A `@Document` with an `OffsetDateTime` field compiles fine and starts the context fine, but `repository.save(...)` throws `CodecConfigurationException: Can't find a codec for ... OffsetDateTime` at runtime — surfaced by ITs, not by unit tests or `mvn compile`. The generated API model classes (`com.example.demo.api.model.*`) DO use `OffsetDateTime` (correct — that's the JSON wire shape with explicit timezone). Convert at the service boundary: persist as UTC `Instant`, return via `.atOffset(ZoneOffset.UTC)`. Cost of getting this wrong: one failed `mvn verify` cycle (~20s).
